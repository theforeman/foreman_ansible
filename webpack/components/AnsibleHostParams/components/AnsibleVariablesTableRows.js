...
const AnsibleVariablesTableRows = ({
  assignedRoles,
  resourceErrors,
  formObject,
}) =>
  assignedRoles.map(role => {
    const overrideKeys = filter(
      role.ansibleVariables,
      ansibleVar => ansibleVar.override
    );

    const firstKey = overrideKeys[0] || {};

    return overrideKeys.map(lookupKey => {
      const resourceError = resourceErrors.find(
        error =>
          lookupKey.id === error.lookupKeyId &&
